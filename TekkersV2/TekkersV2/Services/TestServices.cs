...
namespace TekkersV2.Services
{
    public class TestServices
    {
        ...
        public async Task<List<Test>> GetAssessmentTestsAsync(string assessid)
        {
            TestClient<Test> client = new TestClient<Test>();
        ...
        public async Task PutTestScoreAsync(string id, int testScore)
        {
            TestClient<Test> client = new TestClient<Test>();
        ...
        public async Task<List<Test>> GetTestsForTeamAsync(string teamid)
        {
            TestClient<Test> client = new TestClient<Test>();
            List<Test> teamTests = await client.GetAllTestsForTeamAsync(teamid);
            return teamTests;
        }
    }
}
